...
export default function AppetizersSection() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setError(err.message || 'Failed to load appetizers.');
        setItems([]);
      } finally {
        setLoading(false);
      }
  ...
  if (loading) {
    return <div className="text-center py-40">Loading Appetizers...</div>;
  }
    ...
    <>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        {items.length > 0 ? (
          ...
          <p className="text-center text-gray-600 text-lg">No appetizers are available at the moment.</p>
        )}
      </div>
    </>
  );
